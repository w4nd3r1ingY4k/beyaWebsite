...
const [composeSubject, setComposeSubject] = useState(""); // only used if composeChannel==="email"
  ...
  useEffect(() => {
    ...
    // 2) Load the canonical list of thread IDs with unread counts
    fetch(`${API_BASE}/webhook/threads?userId=${encodeURIComponent(user!.userId)}`)
      ...
      .then(data => {
        ...
        if (data.unreadCounts && Array.isArray(data.unreadCounts)) {
          console.log('🔴 Updating flows with unread counts:', data.unreadCounts);
          
          setFlows(prevFlows => 
            prevFlows.map(flow => {
              const unreadInfo = data.unreadCounts.find((u: any) => u.threadId === flow.flowId);
              if (unreadInfo) {
                return {
                  ...flow,
  ...
  useEffect(() => {
    ...
    fetch(`${API_ENDPOINTS.FLOW_COMMENTS}/flows/${encodeURIComponent(selectedId)}/comments`)
      .then(res => {
...
try {
  postRes = await fetch(
    `${API_ENDPOINTS.FLOW_COMMENTS}/flows/${safeId}/comments`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
