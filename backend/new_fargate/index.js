...
app.post("/debug/intent-classification", async (req, res) => {
  ...
  try {
    // Import the classification function (we need to expose it)
    const { classifyUserIntentDebug } = await import('./services/semantic-search.js');
    
    const result = classifyUserIntentDebug(query, userId || 'test-user');
    ...
    return res.status(200).json({
      ...
      performance: {
        wouldUseDatabase: result.canUseDatabase,
        wouldSkipSemanticSearch: result.canUseDatabase,
        estimatedSpeedGain: result.canUseDatabase ? '10-50x faster' : 'no change'
      }
    });
  } catch (error) {
    console.error("🔥 Intent classification debug error:", error);
    return res.status(500).json({ error: error.message });
  }
});
...
app.get("/debug/vector-metadata", async (req, res) => {
  try {
    ...
    const queryResult = await pineconeIndex.query({
      ...
      topK: 20, // Increased from 10 to see more results
      includeMetadata: true,
      includeValues: false
    });
    
    console.log(`Found ${queryResult.matches.length} vectors in Pinecone`);
    ...
    res.json({
      indexName,
      totalVectors: queryResult.matches.length,
      sampleResults: queryResult.matches.slice(0, 5).map(match => ({
        id: match.id,
        score: match.score,
        metadata: match.metadata,
        metadataKeys: Object.keys(match.metadata || {}),
        hasContent: !!(match.metadata?.content || match.metadata?.chunkableContent || match.metadata?.bodyText),
        emailDirection: match.metadata?.emailDirection,
        eventType: match.metadata?.eventType
      })),
      allEventTypes: [...new Set(queryResult.matches.map(m => m.metadata?.eventType).filter(Boolean))],
      allEmailDirections: [...new Set(queryResult.matches.map(m => m.metadata?.emailDirection).filter(Boolean))],
      fetchAttempt: {
        vectorIds,
        success: !!fetchResult,
...
app.post("/api/v1/query-with-ai", async (req, res) => {
  try {
    ...
    console.log(`🎯 Query-with-AI received:`, { query, userId, userIdType: typeof userId, conversationHistoryLength: conversationHistory?.length || 0 });
...
const server = app.listen(PORT, () => {
  ...
  console.log(`🗄️ Messages Table: ${process.env.MSG_TABLE || 'Messages'}`);
  console.log(`🌎 AWS Region: us-east-1 (hardcoded for DynamoDB queries)`);
  console.log("\nCore Endpoints:");
...
// Debug endpoint to check DynamoDB email contents for debugging company search issues
app.post("/debug/database-emails", async (req, res) => {
  const { userId, limit = 100, searchCompany } = req.body;
  if (!userId) {
    return res.status(400).json({ error: "userId is required" });
  }
  ...
  try {
    ...
    // Query to get recent emails for this user
    const dbQuery = {
    ...
    console.log(`🔍 Debug Database Query:`, JSON.stringify(dbQuery, null, 2));
    
    const result = await docClient.send(new QueryCommand(dbQuery));
    const allEmails = result.Items || [];

    // Analyze the email senders
    const senderAnalysis = {};
    const directionAnalysis = { incoming: 0, outgoing: 0, other: 0 };
    const channelAnalysis = {};
    
    allEmails.forEach(email => {
      // Sender analysis
      const sender = email.From || email.To || 'unknown';
      if (!senderAnalysis[sender]) {
        senderAnalysis[sender] = 0;
      }
      senderAnalysis[sender]++;
      
      // Direction analysis
      if (email.Direction === 'incoming') {
        directionAnalysis.incoming++;
      } else if (email.Direction === 'outgoing') {
        directionAnalysis.outgoing++;
      } else {
        directionAnalysis.other++;
      }
      
      // Channel analysis
      const channel = email.Channel || 'unknown';
      if (!channelAnalysis[channel]) {
        channelAnalysis[channel] = 0;
      }
      channelAnalysis[channel]++;
    });

    // Look for specific company (American Express by default, or user-specified)
    const companyToSearch = searchCompany || 'american express';
    const companyVariations = ['amex', 'american', 'express', 'americanexpress'];
    
    const companyEmails = allEmails.filter(email => {
      const fromField = (email.From || '').toLowerCase();
      const toField = (email.To || '').toLowerCase();
      
      if (searchCompany) {
        // User-specified company search
        const searchTerms = searchCompany.toLowerCase().split(' ');
        return searchTerms.some(term => 
          fromField.includes(term) || toField.includes(term)
        );
      } else {
        // Default American Express search
        return companyVariations.some(variation =>
          fromField.includes(variation) || toField.includes(variation)
        );
      }
    });

    // Show first few emails for detailed inspection
    const sampleEmails = allEmails.slice(0, 10).map(email => ({
      ...
      From: email.From,
      To: email.To,
      Subject: email.Subject ? email.Subject.substring(0, 100) + '...' : 'No subject',
      hasBody: !!email.Body,
      bodyPreview: email.Body ? email.Body.substring(0, 200) + '...' : 'No body'
    }));

    // Check if we hit the limit (indicating there might be more emails)
    const hitLimit = allEmails.length === parseInt(limit);
    const oldestEmailTimestamp = allEmails.length > 0 ? allEmails[allEmails.length - 1].timestamp : null;
    const newestEmailTimestamp = allEmails.length > 0 ? allEmails[0].timestamp : null;
    ...
    return res.status(200).json({
      ...
      paginationInfo: {
        limit: parseInt(limit),
        hitLimit,
        mightHaveMoreEmails: hitLimit,
        oldestReturned: oldestEmailTimestamp,
        newestReturned: newestEmailTimestamp,
        suggestion: hitLimit ? `Increase limit beyond ${limit} to search older emails` : 'All available emails returned'
      },
      analysis: {
        directions: directionAnalysis,
        channels: channelAnalysis,
        topSenders: Object.entries(senderAnalysis)
          .sort(([,a], [,b]) => b - a)
          .slice(0, 20)
          .map(([sender, count]) => ({ sender, count }))
      },
      companySearch: {
        searchedFor: companyToSearch,
        found: companyEmails.length,
        emails: companyEmails.slice(0, 10).map(email => ({
          From: email.From,
          To: email.To,
          Subject: email.Subject,
          Direction: email.Direction,
          timestamp: email.timestamp,
          relativeAge: allEmails.indexOf(email) + 1 // Position in the results (1-based)
        }))
      },
      sampleEmails,
      query: dbQuery
    ...
    console.error("🔥 Database emails debug error:", error);
    return res.status(500).json({ 
      error: error.message,
      stack: error.stack
    });
...
// Debug endpoint to examine raw email structure
app.post("/debug/raw-email-structure", async (req, res) => {
  const { userId, emailCount = 5 } = req.body;
  if (!userId) {
    return res.status(400).json({ error: "userId is required" });
  }
  ...
  try {
    ...
    // Get a few emails to examine structure
    const dbQuery = {
      TableName: process.env.MSG_TABLE || 'Messages',
      IndexName: 'User-Messages-Index',
      KeyConditionExpression: 'userId = :userId',
    ...
    // Analyze the structure of each email
    const emailStructures = emails.map((email, idx) => {
      const structure = {
        index: idx + 1,
        availableFields: Object.keys(email).sort(),
        fieldTypes: {},
        sampleValues: {}
      };
      
      // Check each field type and get sample values
      Object.keys(email).forEach(key => {
        const value = email[key];
        structure.fieldTypes[key] = Array.isArray(value) ? 'array' : typeof value;
        
        // Get sample value (truncated for readability)
        if (typeof value === 'string') {
          structure.sampleValues[key] = value.substring(0, 100) + (value.length > 100 ? '...' : '');
        } else if (Array.isArray(value)) {
          structure.sampleValues[key] = `Array[${value.length}]: ${value.slice(0, 2).join(', ')}`;
        } else if (typeof value === 'object' && value !== null) {
          structure.sampleValues[key] = `Object with keys: ${Object.keys(value).slice(0, 5).join(', ')}`;
        } else {
          structure.sampleValues[key] = value;
        }
      });
    ...
    // Find potential sender/recipient fields
    const potentialSenderFields = new Set();
    const potentialRecipientFields = new Set();
    ...
    emails.forEach(email => {
      Object.keys(email).forEach(key => {
        ...
        // Look for fields that might contain sender info
        if (keyLower.includes('from') || keyLower.includes('sender') || 
            (typeof value === 'string' && value.includes('@') && keyLower.includes('email'))) {
          potentialSenderFields.add(key);
        }
        
        // Look for fields that might contain recipient info
        if (keyLower.includes('to') || keyLower.includes('recipient') || keyLower.includes('receiver')) {
          potentialRecipientFields.add(key);
        }
      });
    });
    ...
    return res.status(200).json({
      ...
      emailStructures,
      fieldAnalysis: {
        commonFields: Object.keys(emails[0] || {}),
        potentialSenderFields: Array.from(potentialSenderFields),
        potentialRecipientFields: Array.from(potentialRecipientFields),
        possibleFromFields: emails.map(e => ({
          From: e.From ? 'Present' : 'Missing',
          from: e.from ? 'Present' : 'Missing',
          sender: e.sender ? 'Present' : 'Missing',
          Sender: e.Sender ? 'Present' : 'Missing',
          from_email: e.from_email ? 'Present' : 'Missing',
          fromEmail: e.fromEmail ? 'Present' : 'Missing'
        }))
      }
    });
